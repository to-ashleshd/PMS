...
//Bug 2
//Display Only M4 to P
    var $arrAllowedGrade = array('M4', 'M5', 'M6', 'M7', 'M8', 'M9', 'M10', 'P');
        ...
        if (!empty($employee_detail)) {
            ...
            foreach ($employee_detail as $key => $val) {
                ...
                if ($relationship_status_detail) {
                    ...
                    if ($submit_status >= '6') {
                        ...
                        if (!empty($overall_rating_by_apraiser)) {
                            $overall_rating_name = getScoreNameByScoreForPreview($overall_rating_by_apraiser[0]['apraiser_score']);
                ...
                if ($submit_status >= 8) {
                    ...
                    $reviewer_assessment_score_name = getScoreNameByScoreForPreview($reviewer_orverall_total);
                }

                //Bug 11 - Bug 20 - Check for time period
                if ($time_period_id >= $val['start_time_period_id']) {


                    //Bug 2 - Allowed specific
                    if (in_array($grade_name, $this->arrAllowedGrade)) {
                        ...
                        $employee_list[] = array(
                            ...
                            'appraiser_overall_score_name' => getScoreNameByScoreForPreview($appraiser_score_details['overall_total']),
    ...
    function getEmployeeApriseelistForReport($pms_employee_id, $status, $time_period_id = 1) {
        //Bug 2
        //Display Only M4 to P
        //$arrowedGrade= array('M4','M5','M6','M7','M8','M9','M10','P');
        //$time_period_id = '1';
...
//        }
    ...
    function getEmployeeApriseelistForKRAReport($pms_employee_id, $status, $time_period_id = 1) {
        //$time_period_id = '1';
        ...
        if (!empty($employee_detail)) {
            ...
            foreach ($employee_detail as $key => $val) {
                ...
                //Get submit status for dual relatinship - Ajay
                foreach ($relationship_status_detail as $key_s => $val_s) {
                    $emp_submit_status = $val_s['submit_status'];
                    //Bug1 fix for dual appraiser
                    if ($val_s['submit_status'] < 1) {
                        //$data['error'] = "Either Your KRA are not filled up or not approved by your Appraiser.";
                        //Reset submit status 
                        $submit_status = $val_s['submit_status'];
                    }
                }
    ...
    function InAuthorizationMatrix($emp_id) {

        $this->db->set_dbprefix('ecore_');
        $sql = "SELECT count(*) as howmany FROM " . ECOREPREFIX . "employee_permission_mapping 
                WHERE pms_employee_id = '" . $emp_id . "' ";
        $query = $this->db->query($sql);
        $result = $query->result();
        $row = $query->first_row();
