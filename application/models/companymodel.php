...
class Companymodel extends CI_Model {
    ...
    {
       $sort_order = $this->get_grade_sort_order($grade_id);
       if(!empty($sort_order))
       {
            $this->db->select('*');
            $this->db->from($this->db->dbprefix . 'grade' );
            $this->db->join($this->db->dbprefix . 'designation', 'designation.grade_id = grade.grade_id','left');
            $this->db->where('grade.sort_order <',$sort_order);
            $this->db->where('status',$grade_status);
            $this->db->where('designation_status',$designation_status);
            $query = $this->db->get();
            return $query->result_array();
    ...
    function get_grade_sort_order($grade_id)
    {
       $sort_order = '';
    ...
    function get_grades_by_sort_order($grade_id)
    {
        //echo $grade_id;die();
       $sort_order = '';
       $grade_ids = array();
       ...
       if(!empty($result))
       {
            ...
            $this->db->select('*');
            $this->db->from($this->db->dbprefix . 'grade' );
            $this->db->where('sort_order <', $result['sort_order']);
            $query1 = $this->db->get();
