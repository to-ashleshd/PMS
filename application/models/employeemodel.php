...
class Employeemodel extends CI_Model {
    ...
    function add_employee($data = array()) {
     ...
     function edit_employee($pms_employee_id,$data = array()) {
        //dbprefix
...
//           die();
    ...
    function get_downgrade_employee_list($grade_id,$status='')
    {
       ...
       $grade_ids = $this->companymodel->get_grades_by_sort_order($grade_id);
       if(empty($grade_ids))
        {
            $grade_ids = array('0');
        }
       ...
       //Restore Changes by ajay with Condition
       $this->db->where('designation.grade_id <=',$grade_id);
       //$this->db->where_in('designation.grade_id',$grade_ids);
       //$this->db->order_by("designation.sort_order", "ASC");
       //Update Sort order
       $this->db->order_by("employee.fname", "ASC");
       $query = $this->db->get();
...
//       echo "</pre>";die();
    ...
    function get_employee_is_in_valid_office_address($pms_employee_id,$office_address_ids = array())
    {
       $employee_detail = array();
       $this->db->select('*');
       $this->db->from($this->db->dbprefix . 'employee' );
       $this->db->where('employee.user_status','1');
       $this->db->where('pms_employee_id',$pms_employee_id);
       $this->db->where_in('employee.office_address_id',$office_address_ids);
       $this->db->join($this->db->dbprefix . 'designation', 'designation.designation_id = employee.designation_id','left');
       $this->db->order_by("designation.sort_order", "ASC");
       $query = $this->db->get();
    ...
    function get_company_list_by_business_and_acl($business_id ='',$pms_employee_id='')
    {
        $this->load->model('companymodel');
           $company_ids = $this->companymodel->get_employee_company_ids_by_acl($pms_employee_id);
            ...
            foreach ($query->result() as $key => $val) {
                $output[] = array('company_master_id'=>$val->company_master_id,'company_name'=>$val->company_name);
            }
    ...
    function get_offices_list_by_company_and_acl($company_id='',$pms_employee_id='')
    {
        $this->load->model('companymodel');
        ...
        if (!empty($result)) {
            foreach ($result as $keye=>$resulte) {
                $office[] = array('office_address_id'=>$resulte->office_addresses_id,'office_name'=>$resulte->office_name);
            }
        }
    ...
    function get_department_by_office_address($office_address_id)
    {
        ...
        if (!empty($result)) {
            foreach ($result as $resulte) {
                $department[] = array('department_id'=>$resulte->department_id,'department_name'=>$resulte->department_name);
            }
        }
    ...
    function get_last_rating($pms_employee_id)
    {
        $data['last_score'] = '';
