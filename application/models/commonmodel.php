...
// echo "</pre>";
    ...
    function get_all_weightage($weightage_value,$status)
    {
       $this->db->select('*');
       $this->db->from($this->db->dbprefix . 'weightage' );
       $this->db->where('weightage_status',$status);
       $this->db->like('LCASE(weightage_value)', $weightage_value, 'after'); 
       $query = $this->db->get();
       $row = $query->result_array();
     ...
     function get_weightage_by_value($weightage_value,$status)
    {
       $this->db->select('*');
       $this->db->from($this->db->dbprefix . 'weightage' );
       $this->db->where('weightage_status',$status);
       $this->db->where('LCASE(weightage_value)',$weightage_value);
       $query = $this->db->get();
       $row = $query->first_row('array');
      // echo $this->db->last_query();die();
       return $row;
    ...
    function add_weightage($data = array())
    {
        $result = $this->db->insert($this->db->dbprefix . 'weightage', $data);
        $id = $this->db->insert_id();
        return $id;
    }
    
    
    function get_all_time_period()
    {
       $this->db->select('*');
    ...
    function get_immediate_previous_year()
    {
       $this->db->select('*');
       $this->db->from($this->db->dbprefix . 'time_period' );
    ...
    function get_allowed_module_settings($module_unique_code)
    {
       $sql = "SELECT * FROM ecore_allowed_module_settings where  LCASE(module_unique_code) like  '".$module_unique_code."%' ";
    ...
    function get_usel_acl_by_controller_name($controller_name,$logged_user_id)
    {
         $sql = "SELECT group_concat(ms.module_unique_code) as allowedurl
                FROM ".ECOREPREFIX."user_acl uacl
                LEFT JOIN ecore_allowed_module_settings ms ON uacl.modulename = ms.id
                WHERE user_id = '" . $logged_user_id . "' AND ms.module_unique_code like '".$controller_name."%' ";
        $query = $this->db->query($sql);
        $result = $query->result_array();
        $row = $query->first_row('array');
        $allowed_url = array();
       
        if(!empty($row['allowedurl']))
        {
            $allowed_url = explode(',', $row['allowedurl']);
        }
        return $allowed_url;
    }
    
    function get_setting_tab_acl($logged_user_id)
    {
         $sql = "SELECT group_concat(ms.module_unique_code) as allowedurl
                FROM ".ECOREPREFIX."user_acl uacl
                LEFT JOIN ecore_allowed_module_settings ms ON uacl.modulename = ms.id
                WHERE user_id = '" . $logged_user_id . "' AND ms.module_unique_code like 'clientadmin%' || ms.module_unique_code like 'adminroles%' || ms.module_unique_code like 'localisation%' || ms.module_unique_code like 'competencies%' || ms.module_unique_code like 'employeepermission' ||  ms.module_unique_code like 'import%' || ms.module_unique_code  like 'announcement' ";
        $query = $this->db->query($sql);
        $result = $query->result_array();
        $row = $query->first_row('array');
        $allowed_url = array();
