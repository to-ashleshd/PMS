...
//konstruktor klasy
    ...
    public function getCountryByName($country_name, $country_id = '') {
        if ($country_id != '') {
            $sql = "Select * from " . $this->db->dbprefix . "countrymaster WHERE countrymaster_id !='" . $country_id . "' AND  LCASE(country_name)='" . mb_strtolower($country_name) . "' limit 1";
        } else {
            $sql = "Select * from " . $this->db->dbprefix . "countrymaster WHERE  LCASE(country_name)='" . mb_strtolower($country_name) . "' limit 1";
        }
    ...
    public function getCountryByIsocodeone($country_code_one, $country_id = '') {
        if ($country_id != '') {
            $sql = "Select * from " . $this->db->dbprefix . "countrymaster WHERE countrymaster_id !='" . $country_id . "' AND LCASE(iso_code_2)='" . mb_strtolower($country_code_one) . "' limit 1";
        } else {
            $sql = "Select * from " . $this->db->dbprefix . "countrymaster WHERE LCASE(iso_code_2)='" . mb_strtolower($country_code_one) . "' limit 1";
        }
    ...
    public function getStateByName($state_name, $country_id, $state_id = '') {
        if ($state_id != '') {
            $sql = "Select * from " . $this->db->dbprefix . "statemaster WHERE statemaster_id !='" . $state_id . "' AND countrymaster_id ='" . $country_id . "' AND  LCASE(state_name)='" . mb_strtolower($state_name) . "' limit 1";
        } else {
            $sql = "Select * from " . $this->db->dbprefix . "statemaster WHERE  countrymaster_id ='" . $country_id . "' AND  LCASE(state_name)='" . mb_strtolower($state_name) . "' limit 1";
        }
        $result = mysql_query($sql);
        $row = '';
        if (!empty($result)) {
    ...
    public function getCityByName($city_name, $country_id, $state_id, $city_id = '') {
        if ($city_id != '') {
            $sql = "Select * from " . $this->db->dbprefix . "citymaster WHERE citymaster_id !='" . $city_id . "' AND countrymaster_id ='" . $country_id . "' AND statemaster_id ='" . $state_id . "' AND  LCASE(city_name)='" . mb_strtolower($city_name) . "' limit 1";
        } else {
            $sql = "Select * from " . $this->db->dbprefix . "citymaster WHERE countrymaster_id ='" . $country_id . "' AND statemaster_id ='" . $state_id . "' AND  LCASE(city_name)='" . mb_strtolower($city_name) . "' limit 1";
        }
...
//        $office_address_id              =  '';//$this->input->post('office_address_id',true);
        ...
        if ($user_entity_master_id != '' || $user_entity_master_id != 0) {
            ...
            foreach ($user_work_info as $key => $val) {
                if ($val['meta_key'] == 'from' && trim($val['meta_value'] != '')) {
                    $w_info[$val['meta_key']] = date('m/d/Y', strtotime($val['meta_value']));
    ...
    function call_update_edu() {
        ...
        if ($user_entity_master_id != '' || $user_entity_master_id != 0) {
            ...
            $meta_key = array('education' => ucwords(strtolower($education)), 'degree' => ucwords(strtolower($degree)), 'from' => $f_dt, 'to' => $t_dt, 'desc' => $desc);
            $response['m_key'] = $meta_key;
            foreach ($meta_key as $key => $val) {
                $user_data = array();
                $user_data = array(
            ...
            foreach ($user_edu_info as $key => $val) {
                if ($val['meta_key'] == 'from' && trim($val['meta_value'] != '')) {
                    $e_info[$val['meta_key']] = date('m/d/Y', strtotime($val['meta_value']));
    ...
    public function getofficeaddresslistbycomany() {
        $response = array();
        $cmp = '';
        $company_id = $this->input->post('company_id', true);
        $this->load->model('companymodel');
        $result = $this->companymodel->getOfficesBycompanyId($company_id);
        if (!empty($result)) {
            foreach ($result as $resulte) {
                $cmp .= "<option value='" . $resulte->office_addresses_id . "' >" . $resulte->office_name . "</option>";
            }
        }
        $response['offices'] = $cmp;
        die(json_encode($response));
    }
    
    

    public function getDepartmentlistbyOfficeAddress() {
        $response = array();
        $department_ids = array();
        $department = '';
        $office_address_id = $this->input->post('office_address_id', true);
        $this->load->model('companymodel');
        $result_dept = $this->companymodel->getDepartmentIdsByOficeAddress($office_address_id);

        foreach ($result_dept as $key => $val) {
        ...
        $result = $this->companymodel->getDepartmentByIDs($department_ids);
        if (!empty($result)) {
            foreach ($result as $resulte) {
                $department .= "<option value='" . $resulte->department_id . "' >" . $resulte->department_name . "</option>";
            }
        }
    ...
    function getcompanylistbybusiness() {
        $business_id = $_POST['business_id'];
        $response = array();
        $output = '';
        $sql = "SELECT cm.* FROM " . $this->db->dbprefix . "company_master cm 
                WHERE cm.company_master_id 
                IN( SELECT company_master_id FROM " . $this->db->dbprefix . "business_to_company where business_id = '" . $business_id . "' )";
        ...
        $response['company_list'] = $output;
        die(json_encode($response));
    }
    
     function old_getcompanylistbybusiness_and_acl() {
           $this->load->model('companymodel');
            $business_id = $_POST['business_id'];
            $pms_employee_id =  $_POST['pms_employee_id'];
            $company_ids = $this->companymodel->get_employee_company_ids_by_acl($pms_employee_id);
            $where_comp  = '0';
           
            if(!empty($company_ids))
            {
                foreach($company_ids as $keyc=>$valc)
                {
                    $where_comp .= ','.$valc;
                }
            }
            ...
            $sql = "SELECT cm.* FROM " . $this->db->dbprefix . "company_master cm 
                    WHERE cm.company_master_id 
                    IN( SELECT company_master_id FROM " . $this->db->dbprefix . "business_to_company where business_id = '" . $business_id . "' ) and 
      ...
      public function pld_getofficeaddresslistbycomany_with_acl() {
        $response = array();
        $cmp = '';
        $company_id = $this->input->post('company_id', true);
        $pms_employee_id = $_POST['pms_employee_id'];
        $this->load->model('companymodel');
        $office_id_by_company_id = $this->companymodel->get_office_by_company_ids(array($company_id));
        $office_ids_by_acl = $this->companymodel->get_employee_office_ids_by_acl($pms_employee_id);
        $where1 = '0';
        $where2 = '0';
        
         if(!empty($office_id_by_company_id))
         {
             foreach($office_id_by_company_id as $key=>$val)
             {
                 $where1 .= ','.$val;
             }
         }
         
         if(!empty($office_ids_by_acl))
         {
             foreach($office_ids_by_acl as $key1=>$val1)
             {
                 $where2 .= ','.$val1;
             }
         }
         
        $sql = "SELECT oa.* FROM " . $this->db->dbprefix . "office_addresses oa 
                    WHERE oa.office_addresses_id 
                    IN(".$where1." ) and 
                   oa.office_addresses_id 
                    IN(".$where2.")";
        $query = $this->db->query($sql);
        //$result = $this->companymodel->getOfficesBycompanyId($company_id);
        $result = $query->result();
         $office = '';      
        if (!empty($result)) {
            foreach ($result as $keye=>$resulte) {
                $office .= "<option value='" . $resulte->office_addresses_id . "' >" . $resulte->office_name . "</option>";
            }
        }
     ...
     function getcompanylistbybusiness_and_acl() {
           $this->load->model('companymodel');
            $business_id = $_POST['business_id'];
            $pms_employee_id =  $_POST['pms_employee_id'];
            $company_ids = $this->companymodel->get_employee_company_ids_by_acl($pms_employee_id);
      ...
      public function getofficeaddresslistbycomany_with_acl() {
        $response = array();
        $cmp = '';
        $company_id = $this->input->post('company_id', true);
        $pms_employee_id = $_POST['pms_employee_id'];
    ...
    //Ajax Function for localisation
