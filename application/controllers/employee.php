...
//
            $data['action']          = base_url('employee/addemployeedata');
...
//        die();
    ...
    function editemployee($pms_employee_id = '')
    {
        $data = array();
        $data['site_name'] = $this->generalesettings->getSiteName();
        $data['logo'] = $this->generalesettings->getImage();
        ...
        //check is employee has access
        $is_allow_employee = $this->is_allow_employee($pms_employee_id);
        if(!empty($is_allow_employee))
        {
            if($is_allow_employee=='Y')
            {
...
//        $employee_busines_id_detail =$this->employeemodel->getUserMeta($pms_employee_id,'business_id');
//        if(!empty($employee_busines_id_detail))
//        {
//          $employee_detail['business_id'] =   $employee_busines_id_detail['meta_value'];
//        }
//        else
//        {
//            $employee_detail['business_id'] =   '0';
//        }
        
        $data['pms_employee_id'] = $pms_employee_id;
        $data['employee_detail'] = $employee_detail;
        
        $data['business_list'] = $this->businessmodel->get_business_list_by_employee_id($this->session->userdata('pms_employee_id'));
        $data['company_list'] = $this->employeemodel->get_company_list_by_business_and_acl($employee_detail['business_id'],$this->session->userdata('pms_employee_id'));
        $data['offices_list'] = $this->employeemodel->get_offices_list_by_company_and_acl($employee_detail['company_master_id'],$this->session->userdata('pms_employee_id'));
        
        //print_r($employee_detail['office_addresses_id']); die();
        $data['department_list'] = $this->employeemodel->get_department_by_office_address($employee_detail['office_addresses_id']);
     
        
        
         $data['designations'] = $this->companymodel->getDesignations();
        
            $data['action']          = base_url('employee/editemployeedata');
            $data['s_date_format'] = $this->commonmodel->get_env_setting('s_date_format');
            $data['current_date'] = date($data['s_date_format']);
            $data['js_date_format'] = get_datepicker_date2($data['s_date_format']);
            $data['req_pwd_strength'] = $this->commonmodel->get_env_setting('ui_slider3_sel');
    ...
    function is_allow_employee($pms_employee_id)
    {
        $this->load->model('employeepermissionmodel');
        $status = '1';
        $is_valid_employee = '';
        ...
        if ($pms_employee_id == '1') {
            ...
            if (!empty($employee_permission_detail)) {
                ...
                //die();
                $employee_detail = $this->employeemodel->get_employee_is_in_valid_office_address($pms_employee_id,$office_address_ids);
     ...
     function editemployeedata() {
        ...
        $result = $this->employeemodel->edit_employee($pms_employee_id,$data);
        //add Meta for business
        $this->employeemodel->updateUserMeta($pms_employee_id, 'business_id', $this->input->post('business_list_dd') ) ;
    ...
    function viewemployee($pms_employee_id)
    {
       ...
       $data = array();
        ...
        $relationship_detail = $this->employeemodel->get_relationship_of_employee($pms_employee_id,$current_year_id);
        
        $relationship_list = array();
        if(!empty($relationship_detail))
        {
            $appraisers = '';
            $reviewer = '';
            foreach($relationship_detail as $key_r=>$val_r)
            {
                
                $apraiser_emp_details = $this->employeemodel->getEmployeeById($val_r['apraiser_employee_id']);
                $reviewer_emp_details = $this->employeemodel->getEmployeeById($val_r['reviewer_employee_id']);
                ...
                if($appraisers=='')
                {
                   $appraisers .= '<span style="word-wrap: break-word;">';
                   $appraisers .= '[ <b>'. $apraiser_employee_name .' </b>'.$apraiser_designatoion.' ]';
                   $appraisers .='</span>';
                }
                else
                {
                ...
                if($reviewer=='')
                {
                   $reviewer .= '<span style="word-wrap: break-word;">';
                   $reviewer .= '[ <b>'. $reviewer_employee_name.'</b>,'.$reviewer_designatoion.' ]';
                   $reviewer .= '</span> ';
                }
                else
                {
        ...
        $data['appraiser'] = $appraisers;
        $data['reviewer'] = $reviewer;
        $data['pms_employee_id'] = $pms_employee_id;
        $data['employee_detail'] = $employee_detail;
        $data['company_detail'] = $this->companymodel->get_company_basic_detail($employee_detail['company_master_id']);
        $data['business_detail'] = $this->businessmodel->get_business_by_id($employee_detail['business_id']);
        $data['last_score']      =  $this->employeemodel->get_last_rating($pms_employee_id);
           ...
           $data['action']          = base_url('employee/editemployeedata');
            ...
            $data['common_js'] = $this->load->view('default/clientadmin/cadmin_common_js', $data, true);
            $data['last_footer'] = $this->load->view('default/clientadmin/cadmin_last_footer', '', true);
            $this->load->view('default/clientadmin/cadmin_view_employee_profile', $data);
        }
