        ...
        if ($this->session->userdata('pms_employee_id')) {
            ...
            if ($year == '') {
                ...
                if ($track_status == '0.1') {
                    ...
                    $data['pending_status'] = 'Your kra Not Approved Please Update and resend KRA.';
                }
                if ($track_status >= '0.2' && $track_status < '1') {
                    $data['appraiser_name_info'] = $apariser_name_info;
                    $data['apariser_name_info_not_approve'] = $apariser_name_info_not_approve;
                    $data['pending_status'] = 'Kra Send to Appraiser For Approvel.';
        ...
        if ($this->session->userdata('pms_employee_id')) {
            ...
            if (!empty($_POST)) {
                ...
                if ($result) {
                    ...
                    echo "KRA Submitted Successfully";
                } else {
...
//                            }
            ...
            if (empty($time_period_detail)) {
                ...
                if (isset($data['error'])) {
                    $data['error'] = $data['error'] . "<br /> Either Your KRA are not filled up or not approved by your Appriser.";
                } else {
                    $data['error'] = "Either Your KRA are not filled up or not approved by your Appriser.";
                }
            } else {
                if ($submit_status >= 2) {
...
//                            }
    ...
    public function addidpdata() {
        ...
        if ($this->session->userdata('pms_employee_id')) {
            ...
            if (!empty($_POST['invE_item'])) {
                ...
                if ($result) {
                    echo "IDP Data Submitted Successfully.";
                } else {
                    echo "Please try again";
                }
            }
        ...
        if (!empty($response['kra_detail'])) {
            ...
            else
            {
                    ...
                    $response['appraiser'] = '';
                            ...
                            if ($no_of_appraiser == '1') {
                                $apariser = '<input type="hidden" name="appraiser_employee_id_'.$apraisee_kra_id.'"  id="appraiser_employee_id_'.$apraisee_kra_id.'" value="' . $data['appraiser_list'][0]['apraiser_employee_id'] . '"  />';
                                $apariser .= $data['appraiser_list'][0]['appraiser_name_with_designation'];
                            } else {
                                $apariser = '<select name="appraiser_employee_id_'.$apraisee_kra_id.'"  id="appraiser_employee_id_'.$apraisee_kra_id.'" >';
                                $apariser .= '<option value="">--Please Select--</option>';
                                if(!empty($data['appraiser_list']))
                                {
                                foreach ($data['appraiser_list'] as $keyal => $valal) {
                                    if($response['kra_detail']['current_apraiser_id']==$valal['apraiser_employee_id'])
                                    {
                                        //$response['appraiser_name'] = $valal['appraiser_name_with_designation'];
    ...
    function updatekradata() {
        ...
        if ($result) {
            $response['msg'] = "Apraisee Kra Updated Successfully.";
        } else {
    ...
    function sendtoapraiser() {
        ...
        $response['kra_detail'] = $this->apraiseemodel->get_kra_by_aproove_status($pms_employee_id, '0', $time_period_id);
...
//            
    ...
    function update_all_weight() {
        ...
        if (!empty($kras_weight)) {
            ...
            if ($flag == 0) {
                $response['msg'] = "Appraisee Kra Weightage Updated Successfully.";
            } else {
                $response['msg'] = "Please Try Again.";
            }
        }
