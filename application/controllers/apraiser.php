...
class Apraiser extends CI_Controller {
    ...
    function statusreport() {
        ...
        if ($this->session->userdata('pms_employee_id')) {
            ...
            //Bug 3 - Authorizaton - all BU Head if he is not a Appraiser or Reviewer
		$this->load->model('reportsmodel');
		$in_authorization_matrix = $this->reportsmodel->InAuthorizationMatrix($this->session->userdata('pms_employee_id') );
                                
            
            

            if ($data['is_employee_apraiser'] == 'Y' || $data['is_employee_reviewer'] == 'Y' || $in_authorization_matrix >= 1) {
        ...
        $employee_ids = $this->reportsmodel->getEmpIdsByEmployee_list($my_employee_list);
**/
                ...
                $this->load->view('default/clientadmin/cadmin_status_report', $data);
            } else {
    ...
    function employee() {
        ...
        if ($this->session->userdata('pms_employee_id')) {
            ...
            if ($data['is_employee_apraiser'] == 'Y' || $data['is_employee_reviewer'] == 'Y') {
                ...
                $data['employees'] = $this->apraisermodel->getEmployeeApriseelist($pms_employee_id, '1');
                $data['reviewer_employees'] = $this->reviewermodel->getEmployeeApriseelist($pms_employee_id, '1');
                $data['approvel_employees'] = $this->apraisermodel->get_kra_approvel_appraisee_list_for_both_year($pms_employee_id, '1');
    ...
    function apraiseeassessment($apraisee_employee_id = '', $default_tab='') {
        $data = array();
        $time_period_id = '1';
        ...
        if ($this->session->userdata('pms_employee_id')) {
            ...
            if ($apraisee_employee_id != '') {
                ...
                if ($pms_employee_id == '1') {
                    ...
                    if(!empty($apraisee_relationship_details))
                    {
                        $reviewer_id_for_score = $apraisee_relationship_details['reviewer_employee_id'];
                    }
                    ...
                    else
                    {
                        ...
                        $data['last_score'] = $this->apraiseemodel->get_employee_last_year_rating($pms_employee_id,$reviewer_id_for_score,$imediate_prev_year);
                    }
                    ...
                    if ($apraisee_relationship_detail['submit_status'] >= '6') {
                        ...
                        $data['overall_kra_score_name'] = getScoreNameByScoreForPreview( $data['overall_kra_score']);
                    ...
                    if($time_period_id==1)
                    {
                        $data['last_score'] = $this->apraiseemodel->get_employee_temp_last_rating($pms_employee_id);
                        //Bug 18
                        $data['top_employee_last_score'] = $this->apraiseemodel->get_employee_temp_last_rating($apraisee_employee_id);
                    }
                    else
                    {
                        $imediate_prev_year = $this->commonmodel->get_immediate_previous_year();
                        ...
                        $data['last_score'] = $this->apraiseemodel->get_employee_last_year_rating($pms_employee_id,$reviewer_id_for_score,$imediate_prev_year);
                        //Bug 18
                        $data['top_employee_last_score'] = $this->apraiseemodel->get_employee_last_year_rating($apraisee_employee_id,$reviewer_id_for_score,$imediate_prev_year);
                    }
                    ...
                    $data['emp_grade'] = $employee_detail->grade_id;
                    ...
                    //     get reviewer score if process completed
                    $data['reviewer_overall_score_name'] = '';
                    if (!empty($apraisee_relationship_detail) && $apraisee_relationship_detail['submit_status'] >= '8') {

                        $reviewer_score = $this->reviewermodel->getReviewerOverAllRatingById($apraisee_employee_id, $apraiser_employee_id, $apraisee_relationship_detail['reviewer_employee_id'], $time_period_id);
                        if (!empty($reviewer_score)) {
                            $data['reviewer_overall_score_name'] = $this->reviewermodel->getScoreNameByScore($reviewer_score['reviewer_score']);
                        }
                    }
            ...
            if (!empty($_POST)) {
                ...
                foreach ($_POST as $key => $val) {
                    ...
                    if ($key == 'idpE_item') {
                        foreach ($val as$keyt => $valt) {
                            $valt = trim($valt);
                            //TODO: Fix - Add only non empty values - Ajay
                            if ($valt != '') {
                ...
                if ($result == 1 && $result_technical == 1 && $result_behavioural == 1) {
                    ...
                    if ($is_first_appraiser != 'Y') {
                        ...
                        //TODO: Added By Ajay - 2 Fields: kra_score, competency_score
                        ...
                        $overall_rating_data = array(
                            ...
                            'kra_score' => $apraiser_kra_score,
                            'competency_score' => $apraiser_competency_score,
                            'comment' => '',
                            'feedback' => '',
                    ...
                    echo "Competencies with IDP rated successfully.";
                } else {
                    echo "Please try again";
                }
            }
    ...
    function addoverallratingdata() {
        ...
        if ($this->session->userdata('pms_employee_id')) {
            ...
            if (!empty($_POST)) {
                ...
                if (isset($_POST['overall_rating_comment'])) {
...
//        }
        ...
        // echo $time_period_id;
        $data['apraisee_employee_id'] = '';
        $data['apraiser_employee_id'] = $this->session->userdata('pms_employee_id');
        $data['time_period_id'] = '';
    ...
    function approvedkra() {
        ...
        if ($emp_rel_mat_id != $employee_relationship_matetix_id) {
            ...
            $html .= '</tr>';
            $html .= '</thead>';
            $html .= '<tbody id="kra_detail">';
            if (!empty($data['kra_detail'])) {
                foreach ($data['kra_detail'] as $key => $val) {

                    if ($total_weight == 0) {
                        $total_weight = $val['weightage_value'];
                    } else {
                        $total_weight += $val['weightage_value'];
                    }
                    $html .= '<tr>';
                    ...
                    $html .= '<td style="text-align:center;width:20px;">' . $val['weightage_value'] . '%' . '</td>';
            ...
            $html .= '<td style="text-align:center; font-weight:bold;"><span id="total_weight">' . $total_weight . '</span> %</td>';
    ...
    function donotapprovkra() {
        ...
        if (!empty($employee_relationship_materix_detail)) {
            $emp_rel_mat_id = $employee_relationship_materix_detail['employee_relationship_materix_id'];
            $apraiser_employee_id = $employee_relationship_materix_detail['apraiser_employee_id'];
            $reviewer_employee_id = $employee_relationship_materix_detail['reviewer_employee_id'];
        }
        if ($emp_rel_mat_id != $employee_relationship_matetix_id) {
            $response['error'] = 'Please Try Agian.';
        } else {
        ...
        //add not approve comment
        $data = array(
            'apraisee_employee_id' => $apraisee_employee_id,
            'apraiser_employee_id' => $apraiser_employee_id,
            'reviewer_employee_id' => $reviewer_employee_id,
            'time_period_id' => $time_period_id,
            'not_approve_comment' => ucfirst($comment),
            'not_approve_status' => '1'
        );

        $result_comment = $this->apraisermodel->add_not_Approve_Comment($data);
    ...
    function geOverallRatingForApraisee($apraisee_id, $time_period_id = 1) {
        ...
        $overall_competency_score = number_format((( ($competency_score * 30 ) / 100) / $no_of_apraisers ), 2, '.', '');
    ...
    function test() {
        ...
        $data['common_js'] = $this->load->view('default/clientadmin/cadmin_main_common_js', $data);
//       $data['last_footer'] = $this->load->view('default/clientadmin/cadmin_last_footer', '', true);
        $this->load->view('default/clientadmin/form_validation', $data);
    }

    function check_is_appraisers_having_kra() {
        //through this code min kra and max weightage depend on each other
        $response = array();
        $apraisee_employee_id = $this->session->userdata('pms_employee_id');
        $apraiser_employee_id = $this->input->post('apraiser_employee_id');
        $time_period_id = $this->input->post('time_period_id');

        $is_all_appraiser = $this->apraiseemodel->get_all_kra($apraisee_employee_id, $apraiser_employee_id, $time_period_id);
        die(json_encode($is_all_appraiser));
    }

    function ajax_get_score_name()
    {
        $response = array();
        $total_kra_score = $this->input->post('overall_score',true);
        $score_name = getScoreNameByScoreForPreview($total_kra_score);
        $response['score_name'] =$score_name;
        die(json_encode($response));
    }
