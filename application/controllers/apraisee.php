...
//             echo "</pre>";die();
            ...
            if(!empty($data['pms_detail']))
            {
                foreach($data['pms_detail']  as $key=>$val)
                {
                    if($total_rating=='')
                    {
                        $total_rating = $val['rating_value'];
                    }
                    else
                    {
                        $total_rating += $val['rating_value'];
...
//            }
    ...
    function get_Kra_by_appraiser_for_edit()
    {
     ...
     function get_average_of_rates()
     {
         $response = array();
         $rate_ids = $this->input->post('rating_ids',true);
         $total_kra = $this->input->post('total_kra',true);
         $total_rate = '';
         $average_rate = '0';
//         echo "<pre>";
//         print_r($rate_ids);
//         echo "</pre>";die();
         if(!empty($rate_ids))
         {
             foreach($rate_ids as $key=>$val)
             {
                 $rating_detail = $this->commonmodel->getRatingByid($val);
                 if(!empty($rating_detail))
                 {
                     if($total_rate=='')
                     {
                         $total_rate = $rating_detail['rating_value'];
                     }
                     else
                     {
                          $total_rate += $rating_detail['rating_value'];
         ...
         if($total_rate!='')
         {
             $average_rate = (($total_rate)/($total_kra));
         }
         
         $response['average_rate'] =  $average_rate;
         die(json_encode($response));
     }
