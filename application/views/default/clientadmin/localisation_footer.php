...
<!-- <script src="<?php echo base_url("assets/clientadmin"); ?>/js/jquery.multi-select.js"></script> -->
...
<script type="text/javascript">
	...
	//Multiseleft Default call- Ajay
	$('#select_list1').multiSelect();
	$('#select_list_business_company').multiSelect();
	...
	//$('#select_list1').multiSelect('select', arr );
...
//start function disabled other state
function calldisabled_general(state,id)
{
...
<!-- start function disabled -->
function calldisabledothercity_general(city,id)
{
...
{
	/** Disabled only one parent functionality - Ajay **/
	var company_id = $("#office_type_company").val();
	//alert('Company id: ' + company_id );
	//Fixed for if company id is blank
	if( company_id == '' ) {
		$('select[id=\'office_parent_ot\']').html('<option value="">- Please Select Company -</option>');
	}
	var url = '<?php echo site_url("ajax/getofficeofcomany"); ?>';
	 $.ajax({
				...
				success: function(response) {
					...
					if(response['company_parent_offices']=='')
					{
						/**
						$('#office_parent_ot').html('');
						$('#office_parent_ot').attr("disabled","disabled");
						$('#lbl_office_parent_ot').html('');
						$('#hiscomany').val('1');
						**/
					}
					else
					{
					//alert(response['cmp']);
						$('#office_parent_ot').removeAttr("disabled","disabled");
						$('select[id=\'office_parent_ot\']').html('<option value="">-- Parent N.A. --</option>');
						
						$('select[id=\'office_parent_ot\']').append(response['company_parent_offices']);
						$('#hiscomany').val('0');
					}
...
function call_country_validation()
{
	...
	if(!is_charwithoutspace('countrycode1','lbl_iso_code1','YES','Country Code'))
	{
		flag=1;
	}
	if(!is_charwithoutspace('countrycode2','lbl_iso_code2','NO','Country Code 2'))
	{
		flag=1;
	}
...
<script type="text/javascript">
var rowperpage = '<?php //echo $row_per_page; ?>';

//alert(rowperpage);

//document.getelementByName('').value = '';
//dt_colVis_Reorder_client_length  
</script>
<script type="text/javascript">
function check_office_name(company_id,parent_office_type_id,office_name_id)
{
		var company_id       = document.getElementById(company_id).value;
		var parent_office_id = document.getElementById(parent_office_type_id).value;
		var office_name      = document.getElementById(office_name_id).value;
		var office_type_id   = document.getElementById('hoffice_type_id').value;
		var is_new_company   = document.getElementById('hiscomany').value;
		//hoffice_nm_status
		if(is_new_company==0 && is_new_company!=1)
		{
			if(parent_office_id=='')
			{
				/** Disabled by ajay
				document.getElementById('lbl_office_parent_ot').innerHTML = 'Please Select Parent Office Type';
				return false;
				**/
			}
			else
			{
				document.getElementById('lbl_office_parent_ot').innerHTML = '';
			}
		}
		
		if(!is_char('newofficetype','lbl_newofficetype','YES','New Office type'))
		{
		...
		{
		//document.getElementById('lbl_office_parent_ot').innerHTML = '';
		var url = '<?php echo site_url("ajax/checkofficetypeexist"); ?>';
		 $.ajax({
					url: url,
					dataType: 'json',
					type: 'POST',
					...
					{
						document.getElementById('hoffice_nm_status').value = '1';
						document.getElementById('lbl_newofficetype').innerHTML = 'Office type already exist';
					}
					else if(response['status']=='3')
					{
						document.getElementById('hoffice_nm_status').value = '3';
						document.getElementById('lbl_newofficetype').innerHTML = 'Office type Name already exist';
					}
					else
					{
						document.getElementById('hoffice_nm_status').value = '0';
						document.getElementById('lbl_newofficetype').innerHTML = '';
					}
				}	
				});
...
<script type="text/javascript">
function call_office_type_validation()
{
		var flag=0;
		var company_id       = document.getElementById('office_type_company').value;
		var parent_office_id = document.getElementById('office_parent_ot').value;
		var office_name      = document.getElementById('newofficetype').value;
		var office_type_id   = document.getElementById('hoffice_type_id').value;
		var is_new_company   = document.getElementById('hiscomany').value;
	
		if((is_new_company==0 || is_new_company=='') && is_new_company!=1)
		{
		
			if(parent_office_id=='')
			{
				/** Disable condition for multiple parent reporting structure
				document.getElementById('lbl_office_parent_ot').innerHTML = 'Please Select Parent Office Type';
				flag=1;
				**/
			}
		...
		check_office_name('office_type_company','office_parent_ot','newofficetype');
		var id = document.getElementById('hoffice_nm_status').value;
		if(id==1 || id==3)
		{
			flag=1;
		}
...
$("#business_list_dd").change(function() {
	...
	{
	var url = '<?php echo site_url("ajax/getcompanylistbybusiness"); ?>';
		 $.ajax({
			url: url,
			dataType: 'json',
			type: 'POST',
...
{
	//Display Company List based on Business
		 ...
		 $.ajax({
			...
			success: function(response) {
				...
				if( response.company_list == '' ) {
					...
					$('select[id=\'office_add_company\']').html(response['company_list']);
				}	
...
function call_office_types()
{
	...
	var url = '<?php echo site_url("ajax/getofficeofcomany"); ?>';
	 $.ajax({
				...
				if(response['company_parent_offices']=='')
				{
					$('#office_type_office_add').html('');
					$('#office_type_office_add').attr("disabled","disabled");
					//$('#lbl_office_parent_ot').html('');
					//$('#hiscomany').val('1');
				}
				else
				{
				//alert(response['cmp']);
				$('#office_type_office_add').removeAttr("disabled","disabled");
								$('select[id=\'office_type_office_add\']').html('<option value="">--Please Select--</option>');
								$('select[id=\'office_type_office_add\']').append(response['company_parent_offices']);
								//$('#hiscomany').val('0');
								}
								}	
    		});
...
<script type="text/javascript">
function call_reported_office()
{
	var company_id = $("#office_add_company").val();
	var office_type_id = $("#office_type_office_add").val();
	var office_address_id = $("#hoffice_address_id").val();

	var url = '<?php echo site_url("ajax/getofficeacoordingcomapnyandpriority"); ?>';
	 $.ajax({
				url: url,
				dataType: 'json',
				type: 'POST',
				...
				if(response['status']==1)
				{
								$('#reported_to_oadd').removeAttr("disabled","disabled");
								$('#reported_to_2_oadd').removeAttr("disabled","disabled");
								$('select[id=\'reported_to_oadd\']').html(response['reported_to_offices']);
								$('select[id=\'reported_to_2_oadd\']').html(response['reported_to_offices_name']);
				}
				else if(response['status']==0)
				{
					$('#reported_to_oadd').html('');
					$('#reported_to_oadd').attr("disabled","disabled");
					$('#reported_to_2_oadd').html('');
					$('#reported_to_2_oadd').attr("disabled","disabled");
				}
...
function call_reported_office_names()
{
	var company_id = $("#office_add_company").val();
	var office_type_id = $("#office_type_office_add").val();
	var reported_office_type_id = $("#reported_to_oadd").val();
	var office_address_id = $("#hoffice_address_id").val();


	var url = '<?php echo site_url("ajax/getofficename"); ?>';
	 $.ajax({
				url: url,
				dataType: 'json',
				type: 'POST',
				data: {
						company_id : company_id,
						office_type_id : office_type_id,
						reported_office_type_id : reported_office_type_id,
						office_address_id :office_address_id
    		          },
				success: function(response) {
				
								$('#reported_to_2_oadd').removeAttr("disabled","disabled");
								$('select[id=\'reported_to_2_oadd\']').html(response['office_name']);
...
function check_company_validation()
{
	var company_id 				= $("#office_add_company").val();
	var office_name 			= $("#officeaddressname").val();
	var office_code 			= $("#officecode").val();
	var office_type_id 			= $("#office_type_office_add").val();
	//var reported_office_type_id = $("#reported_to_oadd").val();
//	var reported_office_name_id = $("#reported_to_2_oadd").val();
	var country_id 				= $("#country_office_add").val();
	var state_id 				= $("#state_office_add").val();
	var city_id 				= $("#city_office_add").val();
	var office_address_id 		= $("#hoffice_address_id").val();

	var msg  ='';
	var url = '<?php echo site_url("ajax/checkofficeaddress"); ?>';
	 $.ajax({
				url: url,
				dataType: 'json',
				type: 'POST',
				data: {
						...
						office_type_id : office_type_id,
						//reported_office_type_id : reported_office_type_id,
						//reported_office_name_id : reported_office_name_id,
						country_id :country_id,
						state_id :state_id,
						city_id : city_id,
						office_address_id :office_address_id
				...
				success: function(response) {
					...
					if(response['office_code_status']==1)
					{
						if(msg!='')
						{
						msg += ', Office code';
					...
					if(response['status']==0 && response['office_code_status']==0 && (response['office_status']==1 || response['office_address_status']==1))
					{
					
						msg += 'Office Address';
					}
					if(response['status']==1 || response['office_code_status']==1 || response['office_status']==1 || response['office_address_status']==1)
					{
						
						document.getElementById('hoffice_add_error_status').value = '1';
						alert(msg+" Already exist");
						return false;
					}
					else
					{
						document.getElementById('hoffice_add_error_status').value = '';
						//return true;
					}
...
<script type="text/javascript">
function call_office_address_validation()
{
	var flag=0;
	////check_company_validation();
		var id = document.getElementById('hoffice_add_error_status').value;
		if(id == 1)
		{
			flag=1;
		}
...
function call_office_dept_listing() {
    var elem = document.getElementById('select_dept_offic').value;
    
	if( elem == -1 ) {
		$('select[id=\'select_from\']').html('<option value="">- Select Department / Office -</option>');			
		$('select[id=\'select_list1\']').html('');
		$('#select_list1').multiSelect(); //Ajay
		$('#select_list1').multiSelect('refresh'); //Ajay	
	}
	
	if (elem === '' || elem == null) {
        alert("please select mapping from to Department or");
        return false;
    } else {
		 ////$('#select_list1').multiSelect(); //Ajay
        var url = '<?php echo site_url("ajax/getdepartmentandofficelist"); ?>';
        $.ajax({
            url: url,
            dataType: 'json',
            type: 'POST',
            ...
            success: function (response) {
                ...
                if (elem == 1) {
					 $('select[id=\'select_from\']').html('');
                    //alert(response['office_addresses']);
                    $('select[id=\'select_from\']').html('');
					$('select[id=\'select_from\']').html('<option value="-1">--Please Select--</option>');
					 
                    $('select[id=\'select_from\']').append(response['department']);
                    ...
                    $('select[id=\'select_from\']').append(response['office_addresses']);
...
$("#select_from").change(function() {
	var select_dept_offic = $("#select_dept_offic").val();
	var select_from = $(this).val();
	////alert('dept office ' + select_dept_offic + ' Seleft from ' + select_from ) ;
	$('select[id=\'select_list1\']').html('');
	
	if( select_dept_offic == '2' )
	{
		//Office to dept
		//alert('select dept or office ' +  select_dept_offic );
		 var url = '<?php echo site_url("ajax/getDepartmentIdsByOffice"); ?>';
		$.ajax({
            url: url,
            dataType: 'json',
            type: 'POST',
            data: {
                office_addresses_id: select_from
            ...
            success: function (response) {
				...
				$('select[id=\'select_list1\']').append(response['nonselect_list']);
				$('#select_list1').multiSelect('refresh'); //Ajay
				...
				$.each(select_items, function(i, item) {
					//alert('Show ' + item );
					$('#select_list1').multiSelect('select', item );	
				});
		...
		var url = '<?php echo site_url("ajax/getOfficeIdsByDept"); ?>';
		$.ajax({
            url: url,
            dataType: 'json',
            type: 'POST',
...
function validate_dept()
{
	...
	var select_dept_offic = document.getElementById("select_dept_offic").value;
	var select_from = document.getElementById("select_from").value;
	var msg = '';
	
	
	if ( select_dept_offic == -1 ) {
		msg += 'Please select Department or Office.\n';		
	}
	
	if ( select_dept_offic == -1 ) {
		msg += 'Please select Whome to map (Department or Office).\n';		
	}
