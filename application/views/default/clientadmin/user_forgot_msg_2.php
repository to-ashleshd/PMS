...
<div id="login-wrapper"  style="margin-top: -134.5px; top: 50%; position: absolute; margin-left: -248px; left: 50%; clear:both">
  <div class="main-col" style=" border-left: 1px solid #C2C2C2;float: left;margin-left: 120px;padding: 10px 20px 10px 10px;position: relative;"> <img src="<?php echo base_url("uploads"); ?>/<?php echo $mylogo; ?>" alt="" class="logo_img" style=" margin:-38px 0 0 -125px;position: absolute;top: 50%;width: 100px; height:auto;" />
	...
	<div class="panel" id="userlogin" style="min-height:200px;">
      ...
      <form id="resetpass" action="<?php echo site_url('recover/updatenewpass'); ?>" method="post" onsubmit="return loginvalidation()">
	  <input type="hidden" name="emp_id" value="<?php echo $empid; ?>" />
        <label for="login_desc"><?php echo $msgdesc; ?></label>
		
		<label for="login_name">New Password</label>
        <input name="password" type="password" id="password" maxlength="15" placeholder="password">
		
		<label for="password2">Confirm Password</label>
        <input name="password2" type="password" id="password2" maxlength="15" placeholder="Confirm password">
		
		<div class="submit_sect">
          <button type="submit" class="btn btn-beoro-3" name="submit" value="reset" >Reset Password</button>
        </div>
...
function loginvalidation()
{
	...
	if( password.length <= 4 ) {
		msg += 'Password length between 5 to 15 characters.\n';
	}
	
	if( password != password2 ) {
		msg += 'Password and confirm password did not matched.\n';
	}
