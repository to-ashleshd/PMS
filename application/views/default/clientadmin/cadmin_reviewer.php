                    ...
                    <div class="span12">
						...
						?>
							...
							<tr class="inv_row">
								...
								<td><textarea class="cls_kra_comments"  name="comments_<?php echo $val['apraiser_kra_id']; ?>" id="comments_kra<?php echo $val['apraiser_kra_id']; ?>"></textarea></td>
               ...
               <?php
				  ...
				  ?>
						...
						elseif(isset($competencies_for_refrence))
						{
							if(!empty($competencies_for_refrence))
							{
								foreach($competencies_for_refrence as $keyc=>$valc)
								{
									...
									<?php
									$weightage_value_total += $valc['weightage_value'];
								}
								?>
								<?php
							}
					...
					<?php foreach( $view_technical_detail as $key_tech=>$val_tech ) : ?>
						...
						<tr><td colspan="7" style="text-align:left; border-right:none; border-top:none;border-bottom:none;"><?php echo $val_tech['technical_area']; ?></td></tr>
						<?php endif; ?>
					...
					<?php endif; ?>
						...
						<tr><td colspan="7" style="text-align:left; border-right:none; border-top:none;border-bottom:none;"><?php echo $val_bev['behavioural_area']; ?></td></tr>
						<?php endif; ?>
			   			  ...
			   			  <table class="table table-bordered">
                                                    ...
                                                    <tbody>
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td style="text-align: center; font-size:12px;"><span style="font-weight:bold;"><?php // echo $appraisee_overall_rating['competency_score']; ?><?php echo $overall_competencies_score; ?></span> <span data-placement="bottom" data-title="Weighted Score" data-content="((Total Final Rating Of Competencies) *(30)) / 100" class="btn btn-mini pop-over" style="font-size:10px;" data-original-title="">&Sigma;</span></td>
                                                        </tr>
                                                        <tr>
                                                            <td colspan="2" style="font-weight:bold;">Overall Performance Score</td>
                                                            <td style="text-align: center; font-size:12px;"><span style="font-weight:bold;" ><?php // echo $appraisee_overall_rating['overall_rating']; ?><?php echo $overall_performance_score; ?> </span> <span data-placement="bottom" data-title="Weighted Score" data-content="( Overall KRA Score + Overall Competencies Score ) by Appraiser(s) " class="btn btn-mini pop-over" style="font-size:10px;" data-original-title="">&Sigma;</span>
															</td>
                                                        </tr>
						   ...
						   <table class="table table-bordered">
                                                    ...
                                                    <tbody>
                                                        ...
                                                        <tr>
                                                            ...
                                                            <!--<td>FEE</td>-->
															<td><span id="apraiser_assessment_score"><?php echo $reviewer_overall_rating['score_rating']; ?></span></td></td>
                                                        </tr>
                                                    </tbody>
                         </table>
						 </div>
...
$(document).ready(function() {
	 ...
	 /** TODO Fix need to apply all coments and rating **/
	 /** Check for apply all competencies **/
	 var total_non_apply_rating = 0;
	 var total_non_apply_comments = 0;
	 var msg = '';
	 $('#tbl_reviewer_kra tr').each(function() {
	 	
		//cls_rate
		//var select_val = $(this).find(".cls_rate").val();
		var rate = $(this).find(".cls_kra_rating").val();
		var comments = $(this).find(".cls_kra_comments").val();
		
		
		//alert(select_val);
		if( rate == '' ) {
			total_non_apply_rating = total_non_apply_rating + 1 ;
		}
		
		if( comments == '' ) {
			total_non_apply_comments = total_non_apply_comments + 1 ;
		}
		
	 });
	 
	 if( total_non_apply_rating >= 1 ) {
	 	msg += 'Please apply ratings for all Competencies.\n' ;
	 }
	 
	 if( total_non_apply_comments >= 1 ) {
	 	msg += 'Please apply comments for all Competencies.\n' ;
	 }
	 ...
	 /** End Check for apply all competencies **/
	...
	//for competencies with idp
	 ...
	 if( total_non_apply_rating >= 1 ) {
	 	alert('Please apply ratings for all Competencies.' );
		return false;
	 }
        ...
        $.ajax({
            ...
            success: function (data) {
				$('#submit').removeAttr('disabled','disabled');
				 var html ='';
				$('#reset').click();
				////alert('Data after addcompetencywithidp ' + data );
				if(data=='Please try again'){
				html += '<div class="alert alert-error">';
                html += '<a class="close" data-dismiss="alert">&times;</a>';
                html += '<strong>Error!&nbsp;</strong>'+data;
				...
				$('#flashmessageoverallrating').html('');
				$('#flashmessageoverallrating').after('');
				$('.alert').hide();
				//$('#content_tab_c').show();
