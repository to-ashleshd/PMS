				  ...
				  <table class="table invE_table table-bordered" id="edit_kra_id" >
					...
					?>
						<tr id="editable_<?=$valekr['apraisee_kra_id']?>">
							...
							<td style="text-align:center;" class="cls_edit_link"><a  href="#" onclick="update_kra('<?=$valekr['apraisee_kra_id']?>','<?=$keyekr+1?>')"  ><i class="icsw16-pencil"></i></a></td>
						</tr>
					<?php
						}
						?>
							...
							success: function(response) {
								if(response.kra_detail.length > 0 )
								{
									...
									for(var i=0; i<response.kra_detail.length; i++)
									{
										...
										html += '<td style="text-align:center;" class="cls_edit_link"><a href="#" onclick="update_kra(\''+response.kra_detail[i]['apraisee_kra_id']+'\',\''+j+'\')" ><i class="icsw16-pencil"></i></a></td>';
										html += '</tr>';
									}
									...
									{
										html += '<tr>';
										html += '<td colspan="8" style="text-align:center"><input type="button" class="btn btn-beoro-3 cls_send_to_apraiser" name="send_to_apraiser" id="send_to_apraiser" value="Send To Appraiser" onclick="send_to_appraiser()" /></td>';
					    				html += '</tr>';
										//$('#div_kra_detail').html('');
					...
					data: {
							...
							success: function(response) {
								...
								if(response.kra_detail)
								{
									...
									html += '<td style="text-align:center;" class="cls_edit_link" ><a href="#" onclick="update_kra(\''+response.kra_detail['apraisee_kra_id']+'\',\''+sr_no+'\')" ><i class="icsw16-pencil"></i></a></td>';
									$('tr#editable_'+kra_id).html(html);
								}
					...
					data: {
							...
							success: function(response) {
								...
								else
								{
									...
									html1 += '<td style="text-align:center">&nbsp;</td>';
							    	html1 += '</tr>';
									html1 += '</tbody>';
									html1 += '</html>';
									$('#kra_detail').html('');
									$('#kra_detail').html(html1);
									//$('#note_for_kra').html('');
								}
							}
				});
											...
											else
											{
												...
												$('#edit_kra_detail tr').each(function() {
																...
																if(weight_id_new!='' && weight_id_new!=undefined && weight_new!='' && weight_new!=undefined)
																{
																		...
																		$(this).find(".cls_display_edit_weight").html(html);
																		$(this).find(".cls_edit_link").html('<a  href="#" onclick="update_kra(\''+kra_id[1]+'\',\''+parseInt(sr_no)+'\')"  ><i class="icsw16-pencil"></i></a>');
