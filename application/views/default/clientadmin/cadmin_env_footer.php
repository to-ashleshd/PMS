...
<script type="text/javascript">
    ...
    $(document).ready(function() {
		...
		if($('#gen_clientreg').length) {
            $("#gen_clientreg").iButton({
                labelOn: "Yes",
                labelOff: "No"
            });
        }
		
		if($('#gen_is_offline').length) {
            $("#gen_is_offline").iButton({
                labelOn: "Yes",
                labelOff: "No"
            });
        }
		
		$("#gen_is_offline").change(function() {
            var checkid = $(this).attr('checked') ;
			if( checkid == 'checked' ) {
				$("#offilne_message").show();
			}
			else {
				$("#offilne_message").hide();
			}
        });
		
		//Check default value for offline
		var checkid = $("#gen_is_offline").attr('checked') ;
		if( checkid == 'checked' ) {
		 	$("#offilne_message").show();
		}
		else {
		...
		$("#s_mailer").change(function() {			
			...
			if( yesno ) {
				...
				//Restore
				$("#s_mailer").val('<?php echo $s_mailer; ?>');
				/**
				if( checkid =='mail' ) {
					$("#s_mailer").val('smtp');
				}
	...
	//Apply rows per page
	//$("#dt_colVis_emailtemplate_length").val('25');
	//document.getElementsByName('dt_colVis_emailtemplate_length').value = '25';
	$('[name="dt_colVis_emailtemplate_length"]').val('25');
	
	
	//check for Internet
	$("#oninternet").click( function() {
		
		var ischecked = $(this).attr('checked') ;
		if( ischecked == 'checked' ) {
			$("#recaptcha_rb").show();
			$("#s_recaptcha").attr('checked',true);
			$("#recaptchainfo").show();	
		}
		else {
			$("#s_default").attr('checked',true);
			$("#recaptcha_rb").hide();
			$("#recaptchainfo").hide();			
		}
	...
	//Check for default
	if( $("#oninternet").length )
	{
		var ischecked = $("#oninternet").attr('checked') ;
...
******************************************/	
	...
	if($('#e_multicompany').length) {
		$("#e_multicompany").iButton({
			labelOn: "Yes",
			labelOff: "No"
		});
	}
	
	if($('#e_isprivate2').length) {
		$("#e_isprivate2").iButton({
			labelOn: "Yes",
			labelOff: "No"
		});
	}

	//Check for logintype select
	$("#logintype").change( function() {
		var checkval = $("#logintype").val();
		//alert('select ' + checkval );
		if( checkval == '-1' )
		{
			resettodefault();
		}
		...
		if( checkval == 'Email' )
		{
			...
			$("#rb_private").show();
			$("#e_is_userverified_req").attr('checked',false);
			$("#e_is_userverified_req").attr('disabled',false);
			
			$("#e_verificationby_email").attr('disabled',false);
			$("#e_verificationby_email").attr('checked',true);
			
			$("#rb_all_option").attr('checked',true);
		...
		if( checkval == 'Mobile' )
		{
			...
			$("#e_verificationby_sms").attr('checked',false);
			$("#e_verificationby_sms").attr('disabled',false);
			$("#e_verificationby_sms").attr('checked',true);		
			
				$("#e_sms_api").attr('disabled',false);
				$("#e_sms_username").attr('disabled',false);
				$("#e_sms_passwd").attr('disabled',false);			
		}
		...
		{
			resettodefault();
			$("#is_login_fb").attr('checked',true);
			$("#is_login_fb").attr('disabled',false);
				
			$("#is_login_twitter").attr('checked',false);
			$("#is_login_twitter").attr('disabled',false);
				
			$("#is_login_linkedin").attr('checked',false);
			$("#is_login_linkedin").attr('disabled',false);
	...
	function show_email_logintype()
		{
			...
			$("#e_verificationby_email").attr('disabled',false);
			$("#e_verificationby_sms").attr('disabled',true);
			$("#e_verificationby_ivr").attr('disabled',true);
			
			
			//$("#rb_all_option").attr('disabled',false);
			
		}
		
	function show_mobile_logintype()
		{
			resettodefault();
			//Show Private domains
			$("#rb_private").hide();
			...
			$("#smsapi_info").show();
			$("#login_sns").hide();
			$("#private_domain_info").hide();
			$("#e_is_userverified_req").attr('disabled',false);
			
			$("#e_verificationby_email").attr('disabled',true);
			$("#e_verificationby_sms").attr('disabled',false);
			$("#e_verificationby_ivr").attr('disabled',true);
			
			
			//$("#rb_all_option").attr('disabled',true);
			
		}	
		
		
	function show_empid_logintype()
		{
			resettodefault();
			//Show Private domains
			$("#rb_private").hide();
	...
	function show_viasns_logintype()
		{
			resettodefault();
			//Show Private domains
			$("#rb_private").hide();
	...
	//apply default 
	var selected_logintype = '<?php echo $e_logintype; ?>';
	if( selected_logintype == 'Email' ) {
		show_email_logintype();
	}
	if( selected_logintype == 'Mobile' ) {
		show_mobile_logintype();
	}
	if( selected_logintype == 'EmpID' ) {
		show_empid_logintype();
	}
	if( selected_logintype == 'auto' ) {
		show_auto_logintype();
	}
	if( selected_logintype == 'viaSNS' ) {
		show_viasns_logintype();
	}
	
	
	if(  $("#logintype").length &&  $("#logintype").val() == -1  )
	{
		resettodefault();	
	}
	
	$("#rb_all_option").click( function() {
		$("#private_domain_list").val('');
		$("#private_domain_list").attr('disabled',true);
	...
	*** Validate functions ***
	**************************/
	function validate_email_logintype()
	{
			var e_is_userverified_req = $("#e_is_userverified_req").attr('checked');
			var e_verificationby_email = $("#e_verificationby_email").attr('checked');
			
			var rb_private_option = $("#rb_private_option").attr('checked');
			var rb_all_option = $("#rb_all_option").attr('checked');
			
			var private_domain_list = $("#private_domain_list").val();
			
			if( e_is_userverified_req == 'checked') {
				//if checked for email
				if( e_verificationby_email == 'checked' ) {
					//Email verification
				}
				else {
					msg += 'Please select Email type as Valid Verification Method.\n';
				}	
			}
			
			if( rb_private_option == 'checked' ) {
				if( private_domain_list.length < 4 ) {
					msg += 'Please enter at list one domain name.\n';
				}
			}
	...
	function resettodefault()
	{
		if( $("#logintype").length ) {		
				...
				//$("#e_isprivate").attr('checked',false);
				$("#e_isprivate").attr('disabled',true);
				
				$("#private_domain_list").attr('disabled',true);
				
				//$("#e_is_sns").attr('checked',false);
				$("#e_is_sns").attr('disabled',true);
				
				//$("#is_login_fb").attr('checked',false);
				$("#is_login_fb").attr('disabled',true);
				
				//$("#is_login_twitter").attr('checked',false);
				$("#is_login_twitter").attr('disabled',true);
				
				//$("#is_login_linkedin").attr('checked',false);
				$("#is_login_linkedin").attr('disabled',true);
				
				//$("#e_is_userverified_req").attr('checked',false);
				//$("#e_is_userverified_req").attr('disabled',true);
				
				//$("#e_verificationby_email").attr('checked',false);
				$("#e_verificationby_email").attr('disabled',true);
				
				//$("#e_verificationby_sms").attr('checked',false);
				$("#e_verificationby_sms").attr('disabled',true);
				
				//$("#e_verificationby_ivr").attr('checked',false);
				$("#e_verificationby_ivr").attr('disabled',true);
				
				$("#e_sms_api").attr('disabled',true);
				$("#e_sms_username").attr('disabled',true);
				$("#e_sms_passwd").attr('disabled',true);
				
				
			//document.getElementById('e_isprivate').disabled=true;
		}	
	...
	if($('#e_is_userverified_req').length) {
		$("#e_is_userverified_req").iButton({
			labelOn: "Yes",
			labelOff: "No"
		});
	}
	
	//If Private domain is checked
	$("#rb_private_option").click( function() {
		$("#private_domain_list").attr('disabled',false);
	});
	
	//Validate Settings
	function validate_env_settings()
	{
		//Pre selected Validation
		/**
		if( $("#logintype").length ) {
		
			var msg = '';
			var selected_logintype = $("#logintype").val();
			//validate_email_logintype();
			alert(selected_logintype );
			return false;
		}
		**/
		var msg = '';
		
		var fromdb = $("#fromdb").val();
		if( fromdb == 1 ) {
			var selected_logintype = '<?php echo $e_logintype; ?>';
		}
		else {
		...
		if( selected_logintype == '-1' ) {
			msg += 'Please select Username Login Type.\n';
			if( msg != '' ) {
				alert(msg);
				return false;
			}	
		...
		if( selected_logintype == 'Email' ) {
			...
			if( rb_private_option == 'checked' ) {
				if( private_domain_list.length < 4 ) {
					...
					var invalid_domains = '';
					var domainlist = private_domain_list.split('\n');
					$.each(domainlist, function(){
						 //alert(this);
						 if( checkdomain(this) == false ) {
						 	invalid_domains += this + '\n'; 
						 }
						 //for check www.
						 if (this.toLowerCase().indexOf("www.") >= 0)
						 {
						 	invalid_domains += this + '\n'; 
						}
					...
					if( invalid_domains != '' ) {
						invalid_domains = 'Invalid Domains: ' + invalid_domains	
					}
					msg += invalid_domains ;
				}
		...
		if( selected_logintype == 'Mobile' ) {
			//show_mobile_logintype();
			var e_sms_api = $("#e_sms_api").val();
			var e_sms_username = $("#e_sms_username").val();
			var e_sms_passwd = $("#e_sms_passwd").val();
			
			var e_is_userverified_req = $("#e_is_userverified_req").attr('checked');
			var e_verificationby_sms = $("#e_verificationby_sms").attr('checked');
			...
			if( e_sms_username == '' ) {
				msg += 'Please enter SMS API Username.\n';
			}
			
			if( e_sms_passwd == '' ) {
				msg += 'Please enter SMS API Password.\n';
			}
			
			
			if( e_is_userverified_req == 'checked') {
				//if checked for email
				if( e_verificationby_sms == 'checked' ) {
					//Email verification
				}
				else {
					msg += 'Please select SMS type as Valid Verification Method.\n';
				}	
			}
		...
		if( selected_logintype == 'viaSNS' ) {
			...
			var is_login_fb = $("#is_login_fb").attr('checked');
			var is_login_twitter = $("#is_login_twitter").attr('checked');
			var is_login_linkedin = $("#is_login_linkedin").attr('checked');
			
			if( is_login_fb == 'checked' || is_login_twitter == 'checked' || is_login_linkedin == 'checked' )
			{
				//Select option
			}
			else {
				msg += 'Please select at least one SNS service.\n';
			}
	...
	/** Validate Database Setting **/
	function validate_db()
	{
		var msg = '';
		var s_db_host = $("#s_db_host").val() ;
		var s_db_user = $("#s_db_user").val() ;
		var s_db_pass = $("#s_db_pass").val() ;
		var s_db_name = $("#s_db_name").val() ;
		
		if( s_db_host == '' ) {
			msg += 'Please enter Database Host Name.\n';
		}
		
		if( s_db_user == '' ) {
			msg += 'Please enter Database User name.\n';
		}
		
		if( s_db_pass == '' ) {
			msg += 'Please enter Database Password.\n';
		}
		...
		if( msg != '' ) {
			...
			var yesno = confirm('CAREFUL !!!\nPlease make sure that you are already copy all data to this new location.\nAre you sure to go?');
			if( yesno ) {
				alert('Continue with existing settings');
			}
			else {
	...
	var url = '<?php echo site_url("ajax/updatecountry"); ?>';
	 $.ajax({
				...
				success: function(response) {
									...
									window.location = '<?php echo site_url('clientadmin/localisation');?>';
								}	
    		});
...
{	
	//var domainExp = '/^(http|https|ftp)://([A-Z0-9][A-Z0-9_-]*(?:.[A-Z0-9][A-Z0-9_-]*)+):?(d+)?/?/i' ;
	//var emailExp = /^[\w\-\.\+]+\@[a-zA-Z0-9\.\-]+\.[a-zA-z0-9]{2,4}$/;
	var domainExp = /^(?:[-A-Za-z0-9]+\.)+[A-Za-z]{2,6}$/;
