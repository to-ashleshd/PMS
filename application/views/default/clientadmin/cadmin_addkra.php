					...
					?>
							...
							if(isset($kra_detail))
							{
								...
								<?php							
									...
									<?php
									$i++;
										$total_weight_todisplay = $total_weight_todisplay + $val['weightage_value'] ;
									} 
									?>
									<tr class="last_row">
										<td colspan="4" style="text-align:right"><b>Total:</b></td>
										<td style="text-align:center; font-weight:bold;"><span id="total_weight"><?php echo $total_weight_todisplay; ?></span> %</td>
...
function send_to_appraiser()
{
	...
	// alert(max_weight);
	 ...
	 else
	 {
		...
		if(t==true)
		{
			$('#send_to_apraiser').attr('disabled','disabled');
		$('#div_edit_all_weight').html('');
		...
		var html = '';
 		 $.ajax({
					...
					data: {
							...
							success: function(response) {
							$('#send_to_apraiser').removeAttr('disabled','disabled');
								if(response.msg)
								{
									html += '<div class="alert alert-success">';
									html += '<a class="close" data-dismiss="alert">&times;</a>';
...
{	
	//Hide Add KRA form
	$("#main_content_add_kra").hide(1000);
	...
	$('#edit_kra_detail tr').each(function() {
				...
				$(this).find(".cls_send_to_apraiser").html('<input type="button" class="btn btn-beoro-3" name="upadte_all_weight" id="upadte_all_weight" value="Update Weightage" onclick="call_add_all_weight()" />');
...
function update_data_edit_all_appraiser()
{
	...
	$('#edit_kra_detail tr').each(function() {
		...
		{
			$('#update_all_appraiser').attr('disabled','disabled');
			...
			var html = '';
			 $.ajax({
						...
						data: {
								...
								success: function(response) {
								$('#update_all_appraiser').removeAttr('disabled','disabled');
									//alert(response.html);
...
function call_add_all_weight() {
    //	return false();
    //Show Add KRA form
    $("#main_content_add_kra").show(1000);

    if (calculate_update_all_weight() == true ) {
        var flag = 0;
        var flag1 = 0;
        var flag2 = 0;
        var min_weightage = $('#min_weightage').val();
        ...
        if (flag == 1 || flag1 == 1) {
			...
			//alert('Max: ' + max_weight + ' total: ' + total_weight + ' used: ' + used_kra + ' Edit total weight: ' + edit_total_weight);
			


            var all_weight = [];
			var mykra_weight = new Array();
			var counter = 0;
            ...
            $('#edit_kra_detail tr').each(function () {
                ...
                if (weight_id != '' && weight_id != undefined && weight != '' && weight != undefined) {
					...
					var my_kra_id = weight_id.replace('txt_upadte_all_weight_','');
					//alert('Weight: ' + weight + ' id: ' + my_kra_id );
					mykra_weight[counter] = weight+'|'+my_kra_id ;
					counter++;
...
function old_call_add_all_weight() {
    //	return false();
    //Show Add KRA form
    $("#main_content_add_kra").show(1000);
